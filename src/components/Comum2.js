...
class Comum extends React.Component{
	...
	linha(a, b, c){
		...
		const key = "th_" + String(this.i) + "_" + String(this.j) + "_";
		if (b !== null) {
			h.push(<th key={ key } className="horario" scope="col">{this.h1[this.i][0] + " às " + this.h1[this.i][1]}</th>)//
			for (const i in a) {
				if (this.t !== 3)
	...
	caso(c) {
		...
		return(
			...
			<table className="">
  				<thead>	      		
						<tr key={String(c) + "_"}>{this.linha(this.s, null)}</tr>
			  	</thead>
			  	<tbody>
