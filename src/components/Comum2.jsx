...
const Comum = (props) => {
    ...
    function renderLinha(numLinha) {
        ...
        return (
            <tr key={`row-${numLinha}`} className="h-16">
                <td className="border p-2 font-bold whitespace-nowrap">{`${h[numLinha][0]} - ${h[numLinha][1]}`}</td>
                {celulas}
            </tr>
        );
    }
    ...
    function renderIntervalo(key) {
        return <tr key={key}><td colSpan={td + 1} className="bg-gray-800 text-white text-center p-2">Intervalo</td></tr>;
    }
                ...
                {pagesToRender.map((_, index) => {
                    ...
                    return (
                        <button
                            ...
                            className={`py-2 px-4 w-10 h-10 ${state.id === actualPageIndex ? 'bg-blue-700 text-white' : 'bg-blue-500 hover:bg-blue-700 text-white'}`}
                        >
                            {actualPageIndex + 1}
                        </button>
                    );
        ...
        <div className={'p-4 flex flex-col items-center'}>
            ...
            <div className="bg-white shadow-md rounded-lg p-4 flex flex-col flex-1 w-[calc(100vw-30px)]">
                ...
                <div className="flex justify-between items-center mb-4">
                    ...
                    {isPrinting ? " " : <div className={`flex items-center gap-2 ${isPrinting ? 'invisible' : ''}`}>
                        {_fun}
                        <button onClick={handlePrint} className="bg-blue-500 hover:bg-blue-700 text-white font-bold py-2 px-4 rounded">Print</button>
                    </div>}
                </div>
