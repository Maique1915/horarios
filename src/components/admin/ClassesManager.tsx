...
export default function ClassesManager() {
    ...
    const fetchSubjects = async () => {
        const { data } = await supabase.from('subjects').select('id, name, acronym, course_id').order('name');
    ...
    const handleSave = async (data: { class_name: string; ho: number[][]; da: (string[] | null)[] }) => {
        ...
        try {
            ...
            if (editingClass) {
                ...
                // Creation Mode: Check for name collision and auto-increment suffix
                const existingNames = classes.map(c => c.className);
                ...
                if (existingNames.includes(className)) {
                    ...
                    while (true) {
                        ...
                        if (!existingNames.includes(tryName)) {
                            newName = tryName;
                            break;
                        }
    ...
    const getSubjectName = (id: number) => {
        ...
        return sub ? `${sub.acronym} - ${sub.name}` : String(id);
    };
            ...
            return (
                <div className={`${stickyClass} bg-white dark:bg-gray-800 p-6 rounded-lg shadow`}>
                    ...
                    <div className="mb-6">
                        ...
                        <select
                            className="w-full p-2.5 border border-gray-300 rounded-lg dark:bg-gray-700 dark:border-gray-600 dark:text-white dark:focus:ring-primary dark:focus:border-primary focus:ring-2 focus:ring-primary/20 focus:border-primary transition-all shadow-sm"
                        ...
                        <p className="mt-1 text-xs text-gray-500">Selecione a disciplina para habilitar o editor.</p>
                    </div>
        ...
        return (
            <div className={`${stickyClass} bg-white dark:bg-gray-800 p-8 rounded-lg shadow text-center text-gray-400 py-20 border-2 border-dashed border-gray-100 dark:border-gray-700`}>
                <div className="flex flex-col items-center gap-3">
                    ...
                    <button
                        onClick={() => { setIsCreating(true); setNewClassSubjectId(null); setNewClassName(''); }}
                    ...
                    <div className="grid grid-cols-1 lg:grid-cols-5 gap-8">
                        ...
                        <div className="lg:col-span-3 flex flex-col gap-6">
                            <div className="rounded-2xl border border-border-light dark:border-border-dark bg-surface-light dark:bg-surface-dark shadow-sm overflow-hidden flex flex-col min-h-[600px]">
                                {/* Filters Toolbar */}
                                <div className="p-4 border-b border-border-light dark:border-border-dark bg-slate-50/50 dark:bg-slate-900/20 grid grid-cols-1 md:grid-cols-12 gap-4">
                                    {/* Subject Filter */}
                                    <div className="md:col-span-5">
                                        <select
                                            value={filterSubjectId || ''}
                                            onChange={(e) => setFilterSubjectId(e.target.value ? Number(e.target.value) : null)}
                                            className="w-full text-sm p-2.5 rounded-lg border border-slate-300 dark:border-slate-700 bg-white dark:bg-slate-900 focus:ring-2 focus:ring-primary/20 focus:border-primary outline-none transition-all"
                                        >
                                ...
                                {/* Table */}
                                <ClassesTable
                                    data={filteredClasses.slice((currentPage - 1) * itemsPerPage, currentPage * itemsPerPage) as ClassTableData[]}
                                    handleEditClass={(cls) => {
