'use client';

import React from 'react';

const LandingFooter = () => {
    return (
        <footer className="bg-surface-light dark:bg-surface-dark border-t border-border-light dark:border-border-dark pt-12 pb-8">
            <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
                <div className="grid grid-cols-1 md:grid-cols-4 gap-8 mb-8">
                    <div className="col-span-1 md:col-span-1">
                        <div className="flex items-center gap-2 mb-4">
                            <div className="bg-primary rounded-md p-1 flex items-center justify-center w-8 h-8">
                                <span className="material-symbols-outlined text-white text-sm">school</span>
                            </div>
                            <span className="font-bold text-lg tracking-tight text-gray-900 dark:text-white">Horários CEFET</span>
                        </div>
                        <p className="text-sm text-gray-500 dark:text-gray-400">
                            Simplificando a gestão acadêmica para estudantes universitários.
                        </p>
                    </div>
                    <div>
                        <h4 className="font-semibold text-gray-900 dark:text-white mb-4">Navegação</h4>
                        <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                            <li><a href="#funcionalidades" className="hover:text-primary transition">Funcionalidades</a></li>
                            <li><a href="#cursos" className="hover:text-primary transition">Cursos</a></li>
                            <li><a href="#depoimentos" className="hover:text-primary transition">Depoimentos</a></li>
                            <li><a href="#precos" className="hover:text-primary transition">Planos</a></li>
                        </ul>
                    </div>
                    <div>
                        <h4 className="font-semibold text-gray-900 dark:text-white mb-4">Recursos</h4>
                        <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                            <li><a href="#funcionalidades" className="hover:text-primary transition">Gera Grade</a></li>
                            <li><a href="#funcionalidades" className="hover:text-primary transition">Horários</a></li>
                            <li><a href="#funcionalidades" className="hover:text-primary transition">Cronograma</a></li>
                            <li><a href="#funcionalidades" className="hover:text-primary transition">Atividades</a></li>
                            <li><a href="#funcionalidades" className="hover:text-primary transition">Perfil</a></li>
                        </ul>
                    </div>
                    <div>
                        <h4 className="font-semibold text-gray-900 dark:text-white mb-4">Links Úteis</h4>
                        <ul className="space-y-2 text-sm text-gray-600 dark:text-gray-400">
                            <li><a href="http://e-computacao.com.br/" target="_blank" rel="noopener noreferrer" className="hover:text-primary transition">E-Computação</a></li>
                            <li><a href="https://alunos.cefet-rj.br/aluno/" target="_blank" rel="noopener noreferrer" className="hover:text-primary transition">Portal do Aluno</a></li>
                            <li><a href="https://cefet-rj.br/" target="_blank" rel="noopener noreferrer" className="hover:text-primary transition">CEFET-RJ</a></li>
                        </ul>
                    </div>
                </div>
                <div className="border-t border-border-light dark:border-border-dark pt-8 flex flex-col md:flex-row justify-between items-center">
                    <p className="text-sm text-gray-500 dark:text-gray-400 text-center md:text-left">
                        © 2026 Horários CEFET. Todos os direitos reservados.
                    </p>
                    <div className="flex space-x-6 mt-4 md:mt-0">
                        <a href="#" className="text-gray-400 hover:text-gray-500 dark:hover:text-gray-300">
                            <span className="sr-only">GitHub</span>
                            <svg aria-hidden="true" className="h-6 w-6" fill="currentColor" viewBox="0 0 24 24">
                                <path clipRule="evenodd" d="M12 2C6.477 2 2 6.484 2 12.017c0 4.425 2.865 8.18 6.839 9.504.5.092.682-.217.682-.483 0-.237-.008-.868-.013-1.703-2.782.605-3.369-1.343-3.369-1.343-.454-1.158-1.11-1.466-1.11-1.466-.908-.62.069-.608.069-.608 1.003.07 1.531 1.032 1.531 1.032.892 1.53 2.341 1.088 2.91.832.092-.647.35-1.088.636-1.338-2.22-.253-4.555-1.113-4.555-4.951 0-1.093.39-1.988 1.029-2.688-.103-.253-.446-1.272.098-2.65 0 0 .84-.27 2.75 1.026A9.564 9.564 0 0112 6.844c.85.004 1.705.115 2.504.337 1.909-1.296 2.747-1.027 2.747-1.027.546 1.379.202 2.398.1 2.651.64.7 1.028 1.595 1.028 2.688 0 3.848-2.339 4.695-4.566 4.943.359.309.678.92.678 1.855 0 1.338-.012 2.419-.012 2.747 0 .268.18.58.688.482A10.019 10.019 0 0022 12.017C22 6.484 17.522 2 12 2z" fillRule="evenodd"></path>
                            </svg>
                        </a>
                    </div>
                </div>
            </div>
        </footer>
    );
};

export default LandingFooter;
