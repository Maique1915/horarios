...
export default class Escolhe{
	...
	constructor(genesis: Materia[], cur: string) {
	...
	exc(): Materia[][] {
		...
		while (i > 0) {
			const f: string[] = i.toString(2).padStart(this.genesis.length, '0').split('')
			i--
	...
	compare(a: Materia[], b: Materia[]): number {
		return b.length - a.length
	}

	merge(a: Materia, b: Materia): Materia {
	...
	existe(c: Materia[], a: Materia): boolean {
		for(const b of c)
			if(a._re === b._re)
				return true
		return false
	...
	colide(b: Materia, a: Materia): boolean {
		for (const i in a._ho) {
			const e = a._ho[i]
			const f = b._ho[i]
