...
const Comum: React.FC<ComumProps> = (props) => {
	const [state, setState] = useState<HomeState>({ b: 0, c: 0, ind: 0, id: 0, materias: props.materias })
	...
	function grade() {
		const arr = []
		const cor = []
		const bd: Materia[][] = [...state.materias]
		const [th, td] = dimencao(_cur)
		
		const aux: Materia[][] = _p.separa ? separa(bd) : bd
		_td = td
		_s = s.slice(0, td)
	...
	function selected(e: React.MouseEvent<HTMLAnchorElement, MouseEvent>): void {
		const checkbox = document.getElementById('section1') as HTMLInputElement
